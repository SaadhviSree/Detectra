...
class DiskImageManager:
    def __init__(self, image_path, mount_base='/tmp/disk_mount/'):
        ...
        self.mount_points = []
        self.ewf_mount = None
        self.recovery_dir = os.path.join(mount_base, 'recovered')
    ...
    def _mount_ewf(self):
        ...
        try:
            # Create mount point for ewf
            self.ewf_mount = os.path.join(self.mount_base, 'ewf')
            os.makedirs(self.ewf_mount, exist_ok=True)
            
            # Mount the E01 image
            subprocess.run([
                'ewfmount',
                '-f', 'raw',  # Use 'raw' format if 'files' fails
                self.image_path,
                self.ewf_mount
            ], check=True)
            ...
            print(f"Successfully mounted E01 image at {self.ewf_mount}")
    ...
    def _run_photorec(self):
        ...
        try:
            ewf_file = os.path.join(self.ewf_mount, 'ewf1')
            
            # First, run PhotoRec in non-interactive mode to recover files
            print("Starting file recovery with PhotoRec...")
            subprocess.run([
                'photorec',
                ...
                ewf_file  # Input file
            ], input=b'Y\nY\nquit\n', check=True)  # Default to search whole disk
            
            print("File recovery completed")
            
        except subprocess.CalledProcessError as e:
            print(f"Error running PhotoRec: {e}")
            # Don't raise the error, allow the program to continue with analysis
    ...
    def cleanup(self):
        ...
        if self.ewf_mount:
            try:
                # Ensure we're not in the mounted directory before unmounting
                os.chdir('/')
                
                # Unmount ewf
                subprocess.run(['fusermount', '-u', self.ewf_mount], check=True)
                print(f"Unmounted {self.ewf_mount}")
                
                # Remove mount point directory
                os.rmdir(self.ewf_mount)
            except subprocess.CalledProcessError as e:
                print(f"Error unmounting {self.ewf_mount}: {e}")
