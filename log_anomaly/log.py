...
def detect_windows_log_anomalies(df, contamination=0.1):
    ...
    Returns:
    DataFrame with anomaly scores and labels, silhouette score, and matplotlib figure
    """
    # Preprocess the data
    ...
    # Create a matplotlib figure object for the plots
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    
    # Plot 1: Temporal Distribution of Anomalies
    axes[0, 0].scatter(processed_data['DateTime'], iso_forest_scores,
                       c=iso_forest_labels, cmap='viridis')
    axes[0, 0].set_title('Temporal Distribution of Anomalies')
    ...
    axes[0, 1].scatter(processed_data['Component_encoded'],
                       processed_data['EventId_encoded'],
                       c=iso_forest_labels, cmap='viridis')
    axes[0, 1].set_title('Component vs EventId Anomalies')
    axes[0, 1].set_xlabel('Component')
    axes[0, 1].set_ylabel('EventId')
    
    # Plot 3: Message Length vs Parameter Count
    axes[1, 0].scatter(processed_data['content_length'],
                       processed_data['parameter_count'],
                       c=iso_forest_labels, cmap='viridis')
    axes[1, 0].set_title('Message Length vs Parameter Count')
    axes[1, 0].set_xlabel('Content Length')
    axes[1, 0].set_ylabel('Parameter Count')
    
    # Plot 4: Anomaly Score Distribution
    axes[1, 1].hist(iso_forest_scores, bins=50, color='blue', alpha=0.7)
    ...
    # Return results, silhouette score, and figure
    return results, silhouette_avg, fig


# Example usage:

data = pd.read_csv(r"C:\Users\Mridu\Downloads\Windows_2k.log_structured.csv")
